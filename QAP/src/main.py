...
def select_file():
    ...
    distances, flows = read_matrix(filename)
...
def generate_report(best, score, elapsed_time):
    problem_size = distances.shape[0]
    init_sol = None
...
def execute():
    ...
    if (distances is None) or (flows is None):
        ...
        if tools_var.get() == "Tabu":
            ...
            params = [cooldown_time, init_temp, distances, flows]
...
def create_report():
    ...
    [n, feas_sol, permutation] = read_report(filename)
...
root = tk.Tk()
root.title("Quadratic Assignment Problem")
